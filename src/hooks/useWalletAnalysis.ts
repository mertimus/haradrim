...
export interface UseWalletAnalysisResult {
  ...
  handleWalletLookup: (address: string) => Promise<void>;
...
export function useWalletAnalysis(): UseWalletAnalysisResult {
  ...
  const handleWalletLookup = useCallback(async (nextAddress: string) => {
    ...
    try {
      const analysis = await getWalletAnalysis(nextAddress);
      if (rid !== lookupRequestIdRef.current) return;
