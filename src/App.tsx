      ...
      <header className="flex-none border-b border-border bg-card/80 px-3 py-1">
        <div className="flex items-center gap-3">
          ...
          <nav className="flex gap-1">
            {(["wallet", "flows", "trace"] as const).map((nextMode) => (
              <button
                key={nextMode}
                onClick={() => {
                  setMode(nextMode);
                  setModeInUrl(nextMode, nextMode === "wallet" || nextMode === "flows" ? analysis.address : "");
                }}
          ...
          <div className="w-full max-w-md">
            <SearchBar
              key={`${mode}:${analysis.address}:header`}
              onSearch={handleSearch}
              loading={analysis.loading}
              defaultValue={analysis.searchDisplayValue}
              autoFocus={Boolean(analysis.address) && mode !== "trace"}
              enableShortcut={Boolean(analysis.address) && mode !== "trace"}
            />
          </div>
        </div>
      </header>
          ...
          {!analysis.address && !analysis.loading ? (
            ...
            <div className="flex flex-1 flex-col overflow-hidden">
              ...
              {analysis.walletError && (
                <div className="flex-none items-center justify-between gap-3 border-b border-destructive/30 bg-destructive/5 px-3 py-2 flex">
